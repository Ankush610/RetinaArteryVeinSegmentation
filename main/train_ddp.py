...
# removing the test split for now
...
if __name__ == "__main__":
    ...
    start_epoch = 0
    num_epochs = 1000
    # lr = 1e-3  # initial training
    lr = 5e-4  # fine-tuning
    early_stopping_patience = 100
    checkpoint_path = f"{path}/checkpoint.pth"
    dataset_path = "/home/omjadhav/ankush/medical_project/Multiclass-Segmentation-in-PyTorch/main/dataset/curated_av_data"
    colormap = [
        [0, 0, 0],        # 0: Background
        [255, 0, 0],      # 1: Artery
        [0, 0, 255],      # 2: Vein
        [0, 255, 0],      # 3: Junction
        [255, 255, 255]   # 4: Uncertain
    ]
    num_classes = len(colormap)   # 5
    ...
    # train_x, train_y = train_x[:125], train_y[:125]  # For faster training, use a subset of the training data
    data_str = f"Dataset Size: Train: {len(train_x)} - Valid: {len(valid_x)}"
    print(data_str)

    """ Data augmentation: Transforms """
    # this is one the fly augmentations here model will get different image in each epoch where p decides the probablity of that transform to be applied
    ...
    """ Optimizer, scheduler, loss function, early stopping """
    ...
    #wrap the model to accelerator for Multi-GPU 
